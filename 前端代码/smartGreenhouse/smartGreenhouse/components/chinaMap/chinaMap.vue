...
<script>
	//建议把json下载到本地
	// import mapdata from '@/config/mapdata.json' //自己的存放路径 我这是在ucharts 实例拷下来的 下面有我的这个文件
	import mapdata from '@/static/chinaMap.json'
	//引用配置文件 用于改写样式覆盖使用
	...
	export default {
		...
		created() {
			...
			uCharts.formatter.tooltipFun = (item, category, index, opts) => {
				...
				return item.data.storeSym ? `${item.name}地区大棚数量：${item.data.storeSym}` : '暂无大棚'
			}
