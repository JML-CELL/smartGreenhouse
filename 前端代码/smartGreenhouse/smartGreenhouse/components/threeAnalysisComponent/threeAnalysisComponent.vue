<template>
	<view class="threeAnalysisComponent">
		...
		methods: {
			getServerData() {
				...
				setTimeout(() => {
					...
					let res = {
						categories: ["04-04", "04-05", "04-06", "04-07", "04-08", "04-09", "04-10"],
						// series: [{
						// 		name: "柱状图",
						// 		index: 1,
						// 		type: "column",
			...
			getIndex(e) {
				console.log(e)
				if (e.event.x >= 26 & e.event.y >= 204 & e.event.x <= 90 & e.event.y <= 270) {

					this.date = '04-04'

					uni.navigateTo({
						url: '/pages/thirdPages/analysisThreeDay/analysisThreeDay?date=' + this.date
					})
				}
			},
		}
