...
def tomato_video_ripe(source, save_dir="./runs/tomato"):
    # 初始化YOLO模型
    model = YOLO("../model/tomato-ripe-best-y8m.pt")

    # # 将设备设置为CPU
    # model.to(torch.device('cpu'))

    # 运行YOLO模型预测
    results = model(source=source, save=True, save_txt=True,augment=True, save_conf=True, save_dir=save_dir)
...
def tomato_video_disease(source, save_dir="./runs/tomato"):
    # 初始化YOLO模型
    model = YOLO("../model/tomato-disease-besty8m.pt")
    # 将设备设置为CPU
    # model.to(torch.device('cpu'))

    # 运行YOLO模型预测
    results = model(source=source, save=True, save_txt=True,augment=True, save_conf=True, save_dir=save_dir, conf=0.6)
