...
public class GptService {
    ...
    private static final String API_KEY = "41ddc24bb8142fb80c47178654960390.dxCGO4RTPlyHicju";
...
//        testSseInvoke();
        ...
        // 2. invoke调用模型,直接返回结果
        testInvoke("种植在温室的草莓幼苗，适宜的温度为多少，土壤湿度为多少？回答的格式为{\"temperature\":,\"soil_moisture\":},只需要回答数字不需要进行描述");
...
//         ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), "你能帮我查询2024年1月1日从北京南站到上海的火车票吗？");
     ...
     * 同步调用
     */
    public static PredictResult testInvoke(String content) throws JsonProcessingException {
        List<ChatMessage> messages = new ArrayList<>();
        ...
        System.out.println("result" + invokeModelApiResp.getData().toString());
//            System.out.println("model output:" + mapper.writeValueAsString(invokeModelApiResp));
        ObjectMapper objectMapper = new ObjectMapper();
        Map<String, Object> data = objectMapper.readValue(mapper.writeValueAsString(invokeModelApiResp), Map.class);

//// 现在，您可以访问和操作转换后的Java对象
//            System.out.println(data.get("code")); // 输出code值
//            System.out.println(data.get("msg")); // 输出msg值

// 访问嵌套对象
        Map<String, Object> dataData = (Map<String, Object>) data.get("data");
//            System.out.println(dataData.get("model")); // 输出model值

// 访问choices数组中的元素
        List<Map<String, Object>> choices = (List<Map<String, Object>>) dataData.get("choices");
        Map<String, Object> choice = choices.get(0);
        Map<String, Object> message = (Map<String, Object>) choice.get("message");
        String contents = (String) message.get("content");
        PredictResult predictResult=new Gson().fromJson(contents, PredictResult.class);
//        System.out.println("这是温度："+predictResult.getTemperature());
//        System.out.println("这是湿度："+predictResult.getSoil_moisture());
//        //
//        System.out.println(contents); // 输出content值
    return predictResult;
    }

    public static String robot(String content) throws JsonProcessingException {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), content);
        messages.add(chatMessage);
        String requestId = String.format(requestIdTemplate, System.currentTimeMillis());
...
// 访问choices数组中的元素
     ...
     * 异步调用
     */
    public static String testAsyncInvoke(String content) {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), content);
        messages.add(chatMessage);
        String requestId = String.format(requestIdTemplate, System.currentTimeMillis());
        ...
        System.out.println("model output:" + JSON.toJSONString(invokeModelApiResp.getData()));
     ...
     * @param taskId
     */
    public static void testQueryResult(String taskId) {
